...
export function doorSpace(door, left) {
    this.door = door;
    this.left = left;
}

export function win(width, height) {
    this.width = width;
    this.height = height;
    this.left = 0;
    this.bottom = 0;
...
export function windowSpace(window, left, top) {
    this.window = window;
    this.left = left;
    this.top = top;
}

export function wall(corner, doorSpaces, windowSpaces) {
    this.corner = corner;
    this.doorSpaces = doorSpaces || [];
    this.windowSpaces = windowSpaces || []; 
}

export function buildFromPlan(walls, ply, height, options, scene) {
    ...
    for(var w = 0; w < nbWalls - 1; w++) {
        ...
        polygonTriangulation = new BABYLON.PolygonMeshBuilder("", polygonCorners, scene, earcut);	
