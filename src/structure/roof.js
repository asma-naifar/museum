...
export function roofprint(corners, overlap, height) {
  var outerData = [];
  var angle = 0;
  var direction = 0;
  var line = BABYLON.Vector3.Zero();
  var lineNormal = BABYLON.Vector3.Zero();
...
var roof = function (roofprint, apexes, planes, rise, height, uvbase, scene) {
  var positions = [];
  var uvs = [];
  ...
  roofMesh.checkCollisions = true;

  return roofMesh;
};

export function roofFloor(roofprint, scene) {
...
export function createRoof(baseData, roofApexData, height, ply, scene) {
  var overhang = 0.2;
  var overlap = ply + overhang;

  var wholeRoofprint = roofprint(baseData, overlap, height);
  roofFloor(wholeRoofprint, scene);
  ...
  return roof(wholeRoofprint, apexes, planes, 2, height, 5.6, scene);
}
