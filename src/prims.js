...
export function creerCamera(name,options,scn,canvas){
...
export function creerSol(name,options,scene){
	...
	let sol = BABYLON.Mesh.CreateGround(name,larg,prof,2.0,scene) ;
...
export function creerMateriauSimple(nom,options,scn){
...
export function creerSphere(nom,opts,scn){
...
export function creerPoster(nom,opts,scn){
	...
	let tableau1 = BABYLON.MeshBuilder.CreatePlane("tableau-" + nom, {width:largeur,height:hauteur}, scn);
	tableau1.parent = group ; 
	tableau1.position.y = hauteur/2.0 ; 

	let mat = new BABYLON.StandardMaterial("tex-tableau-" + nom, scn);
...
export function creerCloison(nom,opts,scn){
...
export function set_FPS_mode(scene, canvas, camera){
